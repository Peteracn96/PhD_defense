...
\begin{columns}[T]
        ...
        \column{.6\linewidth}
            ...
            \vspace{0.3cm}
            \only<4->{\\For $\beta \sim 1$, and $V_0 \sim 40$~meV, $L\sim40$~nm}
            \end{textblock*}
...
\pdfpcnote{We adopt a Wannier description}
\pdfpcnote{because we look at low-q}
\pdfpcnote{therefore low-energy bands}

\pdfpcnote{Trial functions are}
\pdfpcnote{solutions 2D hydrogen atom}
...
\begin{frame}{Absorption Peaks}
    ...
    \only<1->{
\begin{textblock*}{12.0cm}(.2cm,1.0cm)
\small{
...
\only<2-3>{
    \begin{figure}
        ...
        \begin{subfigure}{.5\textwidth}
            \centering
            \transparent{0.5}\includegraphics[scale=0.5]{Figures/Absorption_vs_Resigma.pdf}
        \end{subfigure}
    \end{figure}}
...
\only<4>{
    \begin{figure}
        ...
        \begin{subfigure}{.5\textwidth}
            \centering
            \transparent{0.5}\includegraphics[scale=0.5]{Figures/absorption_of_beta_a.pdf}
        \end{subfigure}%
        \begin{subfigure}{.5\textwidth}
            \centering
...
\begin{itemize}
    \item<3-> Tunable absorption 
    \item<3-> Different response for different polarizations $\Rightarrow$ grid polarizer
    \item<4-> Non-monotonic behaviour of the absorption peak    
\end{itemize}
\end{textblock*}
...
\begin{textblock*}{12cm}(1.6cm,4.5cm)
    \only<3,6>{\vspace{0.18cm}
        \begin{center}
            \includegraphics[scale=0.45]{Figures/polariton_dispersion_relation.jpg}
        \end{center}}

    \only<4>{
        \begin{center}
            \includegraphics[scale=0.45]{Figures/polariton_dispersion_relation_a_&_b.jpg}
        \end{center}}

    \only<5>{
        \begin{center}
            \includegraphics[scale=0.45]{Figures/polariton_dispersion_relation_c.jpg}
        \end{center}}
\end{textblock*}
...
\begin{frame}{Summary}
    ...
    \begin{textblock*}{10cm}(0.5cm,4.6cm)
        \begin{itemize}
            \item<2-> Tunable absorption by changing the polarization
        \end{itemize}
    \end{textblock*}

    \begin{textblock*}{8cm}(.5cm,1.4cm)
        \only<1->{
        \begin{figure}
            \centering
    ...
    \begin{textblock*}{6.2cm}(9.5cm,1.cm)        
        \only<3->{
        \begin{exampleblock}{}
            \small    
            ...
            \begin{itemize}\setlength{\itemsep}{6pt}
                \item<4-> $L \sim 40$~nm, $V_0 \sim 40$~meV feasible
                \item<5-> D.~R.~Danielsen et al. - \textit{ACS Nano} \textbf{19} 22 (2025) $\leftarrow$ experiment with \ch{WS2} % "Fourier-Tailored Light–Matter Coupling in van der Waals Heterostructures"
                \item<6-> G.~Ermolaev et al. - \textit{Arxiv} \textbf{2509} 18866 (2024) $\leftarrow$ experiment with \ch{CrSBr} % "Giant optical anisotropy in CrSBr from giant exciton oscillator strength"
            \end{itemize}
        \end{exampleblock}}
    \end{textblock*}

    \begin{textblock*}{6.2cm}(9.5cm,5.3cm)
        \only<7->{
        \begin{block}{}
            \small    
            However....
            \begin{itemize}\setlength{\itemsep}{6pt}
                \item<8-> Where does $\varepsilon_{\mathrm{RK}}(q)$ come from?
                \item<9-> How to determine $r_0$?
                \item<10-> $\varepsilon_{\mathrm{2D}}(q)$ for higher $q$?
            \end{itemize}
        \end{block}}        
    \end{textblock*}
\end{frame}
