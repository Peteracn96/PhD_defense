...
%     \centering
%     \input{Figures/optical_excitations.tex}
% \end{frame}

\begin{frame}{Optical Excitations in Matter}
    \pdfpcnote{interested in light-matter int. and appli.:}
    \pdfpcnote{Lasing,Polaritonics,Sensoring,imaging}
    \pdfpcnote{exciton: bound electron-hole states}
    \pdfpcnote{explained in the light of quantum physics}
    \pdfpcnote{confer strong optical resonances}
    \pdfpcnote{to insulators and semiconductors}
    \centering
    \input{Figures/optical_materials.tex}
\end{frame}
...
%            }
            ...
            \onslide<3->{
                \begin{itemize}
                    \item<3-> Excitonic series $\neq$ Rydberg series
                    \item<4-> Dielectric "constant" $\varepsilon = \lim_{q \to 0} \lim_{\omega \to 0} \varepsilon (q, \omega) = 1$
                    \item<5-> $\varepsilon (q)$ highly dependent on $q$ (non-locality)
                \end{itemize}
            }
...
\begin{itemize}
    \item<2-> Low defects density [1] %People realized that Luminescent defect centers in hBN were a promising source for SPEs. Defects in hBN are known to reduce phonon lifetimes, and, thus, are detrimental to its use for IR nanophotonics, but such defects are at the heart of hBN’s promise as a single-photon emitter.
    \item<3-> Ballistic transport  in graphene [2] %as an encapsulating material, hBN enhances carriers mobility in graphene. This reports ballistic transport up to 28 micrometer
    \item<4-> Hyperbolic material (MIR) [3]
    \item<5-> Polaritonics in the MIR [4] %hBN interesting on its own as the active material. In this experimental work phonon-polaritons were excited. They used s-snom to measure the propagation length of phonon-polaritons.
\end{itemize}
\end{textblock*}
